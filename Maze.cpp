...
bool Maze::hasConnection(int i1, int j1, int i2, int j2)
{
	...
	{
		return m_field[j1 * m_n + i1].m_down;
	}
...
void Maze::makeConnection(int i1, int j1, int i2, int j2)
{
	...
	if (j1 < j2)
	{
		m_field[j1 * m_n + i1].m_right = true;
	}

	else if (j1 > j2)
	{
		m_field[j2 * m_n + i2].m_right = true;
	}

	else if (i1 < i2)
	{
		m_field[j1 * m_n + i1].m_down = true;
	}

	else
	{
		m_field[j2 * m_n + i2].m_down = true;
	}
};
...
void Maze::removeConnection(int i1, int j1, int i2, int j2)
{
	...
	if (j1 < j2)
	{
		m_field[j1 * m_n + i1].m_right = false;
	}

	else if (j1 > j2)
	{
		m_field[j2 * m_n + i2].m_right = false;
	}

	else if (i1 < i2)
	{
		m_field[j1 * m_n + i1].m_down = false;
	}

	else
	{
		m_field[j2 * m_n + i2].m_down = false;
	}
};
